# ADR-003: Migrate to NatStat Unified Forecasts Endpoint
...
- No access to game status, scores, or enriched metadata
- More complex error handling (partial failures across endpoints)

### Decision Drivers

- NatStat offers a unified `/forecasts` endpoint with all markets
- Need to reduce API call volume to stay within rate limits
- Want guaranteed consistency across all odds markets
- Require game status and scores for auto-scoring feature
- Desire simpler error handling with single request

## Decision

Migrate from three separate NatStat endpoints to the unified `/forecasts` endpoint that provides all betting markets (moneyline, spread, over/under) in a single API call.

### Implementation Approach

1. **Add new functions** - Create `loadForecasts()` and `normalizeForecasts()` alongside existing functions
2. **Rewrite ingestion job** - Update `ingest-natstat.ts` to use unified endpoint
3. **Maintain backward compatibility** - Deprecate but retain old functions for rollback safety
4. **No schema changes** - Use existing database tables with composite unique keys
5. **Add score ingestion** - Leverage game status and scores from forecasts response

### Migration Strategy

- **Zero downtime** - Deploy new code, automatically uses new endpoint
- **No data migration** - Existing games matched by unique composite key
- **Rollback ready** - Old functions preserved if issues arise

## Consequences

### Positive

- ✅ **67% fewer API calls** - Single call replaces three separate requests
- ✅ **Guaranteed data consistency** - All odds from same snapshot timestamp
- ✅ **40-50% faster ingestion** - Eliminated sequential API call overhead
- ✅ **Richer metadata** - Access to ELO ratings, simulation predictions, venue info
- ✅ **Automatic score capture** - No separate job needed for completed games
- ✅ **Simpler error handling** - One request means fewer failure modes
- ✅ **Lower rate limit pressure** - Reduced API quota consumption

### Negative

- ⚠️ **Missing spread/total prices** - Forecasts endpoint doesn't include "juice"
- ⚠️ **Historical data gaps** - Different availability than market-specific endpoints
- ⚠️ **Code duplication** - Temporary maintenance of old and new functions

### Neutral

- 📋 **League availability** - Not all sports supported by forecasts endpoint
- 📋 **Deprecation timeline** - Old functions can be removed after production validation
- 📋 **Testing required** - Verify data completeness for all leagues
