# ADR-005: NatStat Forecasts Endpoint Integration Details

## Status

**Accepted** - Technical reference for ADR-003 implementation

## Context

This document provides the technical specification and usage details for the NatStat `/forecasts` endpoint integration (see ADR-003 for the architectural decision). The forecasts endpoint is NatStat's unified API that consolidates all betting markets into a single response.
...
- Single API call replaces three previous market-specific calls
- Response includes moneyline, spread, and over/under data
- Game status and scores included for completed games
- League code mapping required (standard → NatStat codes)

## Decision

This document serves as the **technical reference** for implementing ADR-003 (Migrate to NatStat Unified Forecasts Endpoint). It defines:

1. API endpoint contract and response structure
2. Data normalization requirements (especially spread sign adjustment)
3. Integration patterns and error handling
4. League code mappings

### Key Technical Decisions

- **Home-relative spreads** - Store all spreads relative to home team (negative = home favored)
- **Favorite team detection** - Use `simulation.prediction-favourite-code` field
- **League code mapping** - Translate standard codes (NFL) to NatStat codes (pfb)
- **Date format** - Support both single dates and ranges (YYYY-MM-DD[,YYYY-MM-DD])

## Consequences

See ADR-003 for architectural consequences. This document focuses on implementation details.

### Technical Implications

- ✅ **Consistent spread display** - UI always shows home team perspective
- ✅ **Simplified scoring logic** - Deterministic spread evaluation
- ⚠️ **League code translation** - Must maintain mapping table
- ⚠️ **Spread sign complexity** - Requires favorite team lookup
