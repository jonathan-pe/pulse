# Point Spread Storage Fix - Implementation Summary

## Problem

Point spreads from NatStat's `/forecasts` endpoint don't explicitly indicate which team they apply to. We need to:

1. Store spreads consistently relative to the home team (negative = home favored)
2. Use NatStat's `spread.favourite` team ID to determine the favored team
3. Cache team ID-to-code mappings to avoid repeated API calls
